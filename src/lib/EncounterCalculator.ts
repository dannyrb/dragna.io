...
class EncounterCalculator {
  ...
  * @returns {void}
  */
  recalculateDifficulty(partyLevel: number, partySize: number, enemyChallengeRatings: number[], allyChallengeRatings: number[]) {
    // Step 1: Scale the Power of each enemy and each ally.
    let totalEnemyPower = 0;
    let totalAllyPower = 0;

    const enemyCrOccurrences = enemyChallengeRatings.reduce(function (acc: Record<number, number>, curr) {
        return acc[curr] ? ++acc[curr] : acc[curr] = 1, acc
    }, {});

    const allyCrOccurrences = allyChallengeRatings.reduce(function (acc: Record<number, number>, curr) {
        return acc[curr] ? ++acc[curr] : acc[curr] = 1, acc
    }, {});
    ...
    Object.keys(enemyCrOccurrences).forEach(function (cr) {
        ...
        const enemiesWithChallengeRating = enemyCrOccurrences[enemenyChallengeRating];
        const enemyPower = EncounterCalculator.CRPowerLookup[enemenyChallengeRating];
        const ratio = enemyPower / EncounterCalculator.LevelPowerLookup[partyLevel];
        const closestRatioMultiplier = _findClosestRatio(ratio, EncounterCalculator.RatioScaleLookup);
        ...
        totalEnemyPower += enemyPower * closestRatioMultiplier * enemiesWithChallengeRating;
    });
    ...
    Object.keys(allyCrOccurrences).forEach(function (cr) {
        ...
        const ratio = allyPower / EncounterCalculator.LevelPowerLookup[partyLevel];
        const closestRatioMultiplier = _findClosestRatio(ratio, EncounterCalculator.RatioScaleLookup);
        // const closestRatio = _findClosestRatio(ratio, EncounterCalculator.RatioScaleLookup);
        // const scaleMultiplier = EncounterCalculator.RatioScaleLookup[closestRatio];

        totalAllyPower += allyPower * closestRatioMultiplier * alliesWithChallengeRating;
    });
  
    // Step 2: Calculate total player + ally Power.
    const partyPower = EncounterCalculator.LevelPowerLookup[partyLevel] * partySize;
    ...
    const hpLost = difficulty;
	  const resourcesSpent = Math.round(0.67 * hpLost);
    const encounterDifficulty = difficultyLevels.find(level => hpLost <= level.max)?.label || 'Unknown';

    ////
    return { hpLost, resourcesSpent, encounterDifficulty };
  }
}
  ...
  */
  const _findClosestRatio = function(targetRatio: number, ratioTable: { ratio: number, multiplier: number }[]) {
